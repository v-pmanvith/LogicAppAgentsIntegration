...
param aiServices string = 'LogicAppsFoundryAgents'

@description('Name for your project resource.')
param firstProjectName string = 'LogicAppsFoundryAgents'

@description('This project will be a sub-resource of your account')
param projectDescription string = 'This project is used to host Logic Apps Foundry Agents.'

@description('The display name of the project')
param displayName string = 'LogicAppsFoundryAgents'
...
module validateExistingResources 'modules-standard/validate-existing-resources.bicep' = {
  name: 'LA-validate-existing-resources-${uniqueSuffix}-deployment'
  params: {
...
module aiDependencies 'modules-standard/standard-dependent-resources.bicep' = {
  name: 'LA-dependencies-${accountName}-${uniqueSuffix}-deployment'
  params: {
    location: location
    ...
    //keyvaultname: 'LA-kv-${name}-${uniqueSuffix}'
...
module aiAccount 'modules-standard/ai-account-identity.bicep' = {
  name: 'LA-ai-account-${accountName}-${uniqueSuffix}-deployment'
  params: {
...
module aiProject 'modules-standard/ai-project-identity.bicep' = {
  name: 'LA-ai-project-${projectName}-${uniqueSuffix}-deployment'
  params: {
    // workspace organization
...
module storageAccountRoleAssignment 'modules-standard/azure-storage-account-role-assignment.bicep' = {
  name: 'LA-storage-${azureStorageName}-${uniqueSuffix}-deployment'
...
module cosmosAccountRoleAssignments 'modules-standard/cosmosdb-account-role-assignment.bicep' = {
  name: 'LA-cosmos-account-role-assignments-${projectName}-${uniqueSuffix}-deployment'
...
module aiSearchRoleAssignments 'modules-standard/ai-search-role-assignments.bicep' = {
  name: 'LA-ai-search-role-assignments-${projectName}-${uniqueSuffix}-deployment'
...
module addProjectCapabilityHost 'modules-standard/add-project-capability-host.bicep' = {
  name: 'LA-capabilityHost-configuration-${projectName}-${uniqueSuffix}-deployment'
  params: {
    accountName: aiAccount.outputs.accountName
...
module cosmosContainerRoleAssignments 'modules-standard/cosmos-container-role-assignments.bicep' = {
  name: 'LA-cosmos-role-assignments-${uniqueSuffix}-deployment'
  scope: resourceGroup(cosmosDBSubscriptionId, cosmosDBResourceGroupName)
  params: {
...
module storageContainersRoleAssignment 'modules-standard/blob-storage-container-role-assignments.bicep' = {
  name: 'LA-storage-containers-${uniqueSuffix}-deployment'
  scope: resourceGroup(azureStorageSubscriptionId, azureStorageResourceGroupName)
  params: { 
...
//  Deploy Logic App that will connect to the Foundry Agent
module logicApp 'modules-standard/logic-app.bicep' = {
  name: 'LA-logicapp-${projectName}-${uniqueSuffix}-deployment'
  params: {
    logicAppName: 'LA-foundryAgentLogic'
    location: location

    // Placeholder values until agent endpoint exists
    foundryEndpoint: ''
    foundryApiKey: ''
  }
  dependsOn: [
